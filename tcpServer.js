...
function TCPModbusServerRaw(options, callbackRequest) {
    var context = this;
    context.listenOptions = options.listenOptions?options.listenOptions:{port: 502};
    ...
    console.log(new Date().toISOString() + ' Modbus TCP server is waiting for connections at:' + JSON.stringify(context.listenOptions));
}
