...
function connect(context){
    context.socket = Net.createConnection({port: context.options.port, host:context.options.host}, function() {
        console.log(new Date().toISOString() + context.namePad + ' connected to serial server');
        ...
        context.socket.on('data', function(data) {
            if (!context.requestActive) {
                ...
                console.log(new Date().toISOString() + context.namePad + ' serial data: ' + data.toString('hex'));
            }
...
RtuClient.prototype.request = function(data, callback) {
    var context = this;
